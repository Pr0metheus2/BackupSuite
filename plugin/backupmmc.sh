...
export HARDDISK=0
echo -n $YELLOW
$SHOW "message35"   	#echo "Full back-up to the MultiMediaCard"
FREESIZE_0=0				
TOTALSIZE_0=0
MEDIA=0
...
if [ -f /mmc/mmc-check ] ; then  
	...
	echo -n $WHITE
	/usr/lib/enigma2/python/Plugins/Extensions/BackupSuite/backupsuite.sh /mmc
	rm -f /mmc/mmc-check
	sync
else
	for candidate in /dev/mmcblk0 /dev/mmcblk1
	do
